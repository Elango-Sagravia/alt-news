...
export async function GET(request) {
  ...
  try {
    ...
    const checkSubscriberQuery = `
      SELECT * FROM subscribers WHERE user_id = $1 AND website_id = 2;
    `;
    const subscriberResult = await query(checkSubscriberQuery, [userId]);

    if (subscriberResult.rows.length > 0) {
      // The user is already in the subscribers table for website_id = 2, update the status to 'subscribed'
      ...
      // The user is not subscribed to website_id 2, insert a new row with created_at
